...
/**
 * Convert genotypes in the BIMBIM dosage format to standardized genotypes with
...
// compile 
// g++ -O2 -std=c++0x -o stdgt standardize_gt.bimbam.cpp
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 4) {
    ...
    cerr << "Usage: " << argv[0] << " <N> <M> <outprefix>" 
	 << endl; 
  ...
  while (getline(cin, line)) {
    ...
    */
    ...
    // col #2 is SNPID
    const char *next_delim_pt = strchr(cstr_pt, ' ');

    int len_SNPID = next_delim_pt - cstr_pt;

    assert(len_SNPID < 128);

    memcpy(snpID, cstr_pt, len_SNPID); 
